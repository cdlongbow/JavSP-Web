...
class GenreMap(dict):
    ...
    def __init__(self, file):
        genres = {}
        csv_path = Path(resource_path(file))

        # 当挂载的 /app/data 目录为空时，容器内可能不存在 genre 映射文件。
        # 为降低用户配置成本：如果文件缺失，则自动在该路径下创建一个仅包含表头的空 CSV。
        if not csv_path.exists():
            try:
                ...
                logger.warning("无法在路径 '%s' 创建 genre 映射文件，将直接使用站点原始分类标签", csv_path)

        if not csv_path.exists():
            # 仍然不存在时，保持空映射表，避免运行时崩溃
            self.update({})
            return
