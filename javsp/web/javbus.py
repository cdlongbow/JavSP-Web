...
def parse_data(movie: MovieInfo):
    ...
    # 页面结构有可能发生变化，直接取下标 0 容易触发 IndexError，这里增加健壮性检查
    container_list = html.xpath("//div[@class='container']")
    if not container_list:
        raise WebsiteError(f"{__name__}: 页面结构异常或访问被拦截，未找到 container: '{movie.dvdid}'")
    container = container_list[0]

    title_nodes = container.xpath("h3/text()")
    if not title_nodes:
        raise WebsiteError(f"{__name__}: 页面结构异常，未找到标题: '{movie.dvdid}'")
    title = title_nodes[0]

    cover_nodes = container.xpath("//a[@class='bigImage']/img/@src")
    if not cover_nodes:
        raise WebsiteError(f"{__name__}: 页面结构异常，未找到封面: '{movie.dvdid}'")
    cover = cover_nodes[0]
    ...
    if not info_list:
        raise WebsiteError(f"{__name__}: 页面结构异常，未找到信息区域: '{movie.dvdid}'")
    info = info_list[0]
